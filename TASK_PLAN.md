...
最後更新：2026-05-26（R-3 第四次訓練進行中；新增 6 個算法 pause/resume 功能）
...
### R-3：MBPO — Pendulum 結果偏差 🔄 第四次訓練進行中
...
**第四次訓練狀態（2026-05-26）**：
- 已執行 ~136K 步，關機中止
- 當前最佳：-1271.5（step 130K），對比第三次 -1323.2 改善 4%
- 配置已優化：慢速 rollout 排程 + 固定 alpha=0.2
- **pause/resume 已實作**：回家後直接 `python train.py` 即可從 130K 繼續

**接續訓練指令**（回家後）：
```powershell
cd C:\Users\666\Desktop\RL\05_Model_Based\2020_MBPO
# 程式自動偵測 checkpoints/resume/ 並接續
C:\Users\666\Desktop\RL\venv\Scripts\python.exe train.py
```
