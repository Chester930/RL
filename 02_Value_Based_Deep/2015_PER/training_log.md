...
| 指標 | 數值 |
|---|---|
| **最佳 eval 回報** | **490.1（步數 180K）** |
| 使用 checkpoint | `best_checkpoints/per_dqn.pt` |
| 關鍵超參修正 | PER α：0.6 → **0.4**（第二次重跑，峰值從 99 → 490）|

> **注意**：CartPole 長訓練（>180K 步）有已知不穩定性，峰值後策略崩潰是正常現象。
> 下方曲線為完整 300K 參考記錄，展示崩潰過程；課堂請使用 `best_checkpoints/` 中的峰值模型。
