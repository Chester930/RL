"""
在 FrozenLake 環境訓練 SARSA 代理人，並在同目錄產生 training_log.md。
...
def _log_episode_detail(ep_num, steps, alpha, gamma):
    ...
    steps: list of (s, a, r, s_next, a_next, delta, q_sa, q_snext_anext)
      q_sa           : Q[s, a]        更新前
      q_snext_anext  : Q[s′, a′]      更新前（終止時為 0）
      a_next         : ε-greedy 實際選的下一個動作
    """
    lines = []
    ...
    # ② 每步即時更新
    lines.append("**② 每步即時更新（同策略 TD 更新）：**")
    lines.append("")
    lines.append(
        "> 公式：δ = r + γ × Q(s′, **a′**) − Q(s,a)　　"
        "a′ 是 ε-greedy 「實際選的」下一動作，不一定是 Q 值最高的——這就是「同策略」。"
    )
    lines.append("")
    lines.append(
    ...
    for t, (s, a, r, s_next, a_next, delta, q_sa, q_sn) in enumerate(steps):
        ...
        is_done = (t == n - 1)
        next_a_str  = "—（終止）" if is_done else _ARROW[a_next]
        q_sn_str    = "0.0000（終止）" if is_done else f"{q_sn:.4f}"
        dq = alpha * delta
        lines.append(
    ...
    if success:
        ...
        lines.append(
            "> 但若 a′ 是隨機探索選到的差動作，Q(s′, a′) 會比 max Q(s′) 低，"
            "> δ 也會跟著低——SARSA 的 Q 值反映的是「帶著探索的真實策略價值」，而非最優策略。"
        )
    lines.append("")
    return "\n".join(lines)
...
def train(config: dict) -> SARSAAgent:
    ...
    for episode in range(1, config["n_episodes"] + 1):
        ...
        # 指數 ε 衰減（與 MC、TD(λ)、Q-Learning 一致）
